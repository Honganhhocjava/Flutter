...
class _MyListViewWithRefreshState extends State<MyListViewWithRefresh> {
  ...
  Future<void> _handleRefresh() async {
    ...
    setState(() {
      items = List.generate(10, (index) => index + 10); // Cập nhật danh sách
    });
  }
...
class _DataLoadingScreenState extends State<DataLoadingScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: FutureBuilder<String>( // <-- Sử dụng FutureBuilder
              ...
              builder: (context, snapshot) {
                ...
                switch (snapshot.connectionState) {
                  case ConnectionState.none:
                    return const Text('Chưa có Stream.', style: TextStyle(fontSize: 18));
                  case ConnectionState.waiting:
                    return const Column(
                      ...
                      children: [
                        CircularProgressIndicator(),
                        SizedBox(height: 16),
                        Text('Đang chờ giá trị đầu tiên...', style: TextStyle(fontSize: 18)),
                      ],
                    );
            ...
            StreamBuilder<int>(
              ...
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  ...
                  return Text('Đã xong: ${snapshot.data}', style: const TextStyle(color: Colors.green));
                }
